...
kbl(amanida_result@stat |>
  ...
  kable_styling(latex_options = c("basic", "scale_down")) |>
  scroll_box(width = "100%", height = "400px") |>
  kableExtra::footnote(symbol = c("Value over cut-offs"))
```
...
Retrieved information for each compound from public databases:

```{r, echo = F}
kbl(amanida_result@stat |>
      arrange(pval) |> 
      group_by(id) |>
      select(- c(pval, trend, fc, N_total))|>
      rename("ID" = id,
      ...
      booktabs = T, 
      caption = "Table 23: Compound complete information",
    align = "c") |>
  kable_styling(latex_options = c("basic", "scale_down")) |>
  scroll_box(width = "100%", height = "400px")
```
